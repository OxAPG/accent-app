...
export default function AccentRoaster() {
  ...
  const handleRecordingStop = async () => {
    ...
    if (chunksRef.current.length === 0) {
      setError("Silence detected. Don't be shy.");
      setStep('landing');
      return;
    }
    const audioBlob = new Blob(chunksRef.current, { type: 'audio/webm' });
    if (audioBlob.size < 5000) { // Small size check for silence
      setError("Too quiet. Actually speak next time.");
      setStep('landing');
      return;
    }
  ...
  const sendToApi = async (base64Audio: string) => {
    try {
      ...
      setError("AI Error. The roast was too hot.");
      setStep('landing');
    }
  };
      ...
      {step === 'recording' && (
        <div className="text-center w-full max-w-sm animate-in zoom-in-95 duration-300">
          ...
          <button 
            ...
            className="w-full mt-10 bg-[#00FF00] border-4 border-black py-4 text-2xl font-black uppercase shadow-[6px_6px_0px_#000] active:translate-y-1 active:shadow-none transition-all hover:bg-red-500 hover:text-white"
          >
            DONE RECORDING ⏹️
          </button>

          <p className="text-[10px] font-black mt-8 uppercase tracking-[0.2em] animate-pulse">
            {timer > 7 ? "AI IS LISTENING..." : "WRAP IT UP, CHAT."}
          </p>
        </div>
      )}
